angularFormsApp.directive('showErrors', function($timeout){
	return {
		...
		link: function($scope, el, attrs, formCtrl){
			...
			// convert the natvie text box element to an angular element
			...
			// include above line if not using ng-if

			// only apply the has-error class after the user leaves the text box
			inputNgEl.bind('blur', function(){
				el.toggleClass('has-error', formCtrl[inputName].$invalid);
				// helpText.toggleClass('hide', formCtrl[inputName].$valid);
				// include the above line if not using ng-if directive
			}); 

			$scope.$on('show-errors-event', function(){
				el.toggleClass('has-error', formCtrl[inputName].$invalid);
			}); // toggle show errors if use clicks submit with form validation errors
